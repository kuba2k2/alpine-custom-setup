...
The required Wi-Fi firmware is automatically installed in the `apkovl`.
...
## Extra: built-in LED display

To enable support for the built-in LED clock and status display, add the following line to `/boot/extlinux/extlinux.conf`:

```
FDTOVERLAYS /boot/dtbs-acs/tm16xx-display-fd628.dtbo
```

To automatically start the [default `display-service`](https://github.com/jefflessard/tm16xx-display/blob/main/display-service), enable the service unit:

```
rc-update add display boot
```
